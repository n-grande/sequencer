...
`;
const BasslineSequencer = ({ bpm, isPlaying, swing, currentStep }) => {
    // Initialize steps with all notes off (velocity: 0)
    const [steps, setSteps] = useState(Array(16).fill({
        note: 0,
    ...
    const [noteOctaves, setNoteOctaves] = useState({});
    ...
    useEffect(() => {
        ...
        const newSequencer = new Tone.Sequence((time, step) => {
            ...
            if (currentStep.velocity > 0 && synthRef.current) {
                ...
                synthRef.current.triggerAttackRelease(noteName, "16n", time, currentStep.velocity);
            }
        }, [...Array(16).keys()], '16n');
    ...
    const handlePitchClick = (noteIndex) => {
        setSelectedPitch(noteIndex);
    };
    const handleOctaveChange = (noteIndex, octave) => {
        setNoteOctaves(prev => ({
            ...prev,
            [noteIndex]: octave
        }));
    ...
    return (_jsxs(Panel, { children: [_jsxs(ControlsContainer, { children: [_jsxs(ControlGroup, { children: [_jsxs(ControlLabel, { children: ["Cutoff:", _jsx(Slider, { type: "range", min: "20", max: "20000", value: filterCutoff, onChange: (e) => setFilterCutoff(Number(e.target.value)) })] }), _jsxs(ControlLabel, { children: ["Resonance:", _jsx(Slider, { type: "range", min: "0", max: "20", value: filterResonance, onChange: (e) => setFilterResonance(Number(e.target.value)) })] })] }), _jsxs(ControlGroup, { children: [_jsxs(ControlLabel, { children: ["Decay:", _jsx(Slider, { type: "range", min: "0.1", max: "1", step: "0.1", value: decay, onChange: (e) => setDecay(Number(e.target.value)) })] }), _jsxs(ControlLabel, { children: ["Envelope Mod:", _jsx(Slider, { type: "range", min: "0", max: "1", step: "0.1", value: envelopeMod, onChange: (e) => setEnvelopeMod(Number(e.target.value)) })] })] }), _jsx("button", { onClick: testSynth, style: {
                            padding: '8px 16px',
                            background: retroAccent,
                            color: '#222',
                            border: 'none',
                        ...
                        }, children: "Test Synth" }), _jsx(ClearButton, { onClick: clearSequencer, children: "Clear Bassline" })] }), _jsx(PitchGrid, { children: PITCH_GRID.NOTES.map((note, index) => (_jsxs(NoteContainer, { children: [_jsx(PitchButton, { active: selectedPitch === index, isCurrent: false, onClick: () => setSelectedPitch(index), children: note }), _jsx(OctaveControl, { children: PITCH_GRID.OCTAVES.map((octave) => (_jsx(OctaveButton, { active: noteOctaves[index] === octave, onClick: () => handleOctaveChange(index, octave), children: octave }, octave))) })] }, note))) }), _jsx(SequencerGrid, { children: steps.map((step, index) => (_jsxs(StepButton, { active: step.velocity > 0, current: currentStep === index, onClick: () => handleNoteClick(index), children: [PITCH_GRID.NOTES[step.note], step.octave, _jsx(SlideButton, { active: step.slide, current: currentStep === index, onClick: () => handleSlideToggle(index) })] }, index))) })] }));
};
